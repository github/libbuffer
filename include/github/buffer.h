...
/**
 * Adjust the length of data in the buffer, within the allocated limit.
 *
 * The declared length of the data will be updated, and a '\0' terminator
 * added.  There must be sufficient room in the underlying allocated
 * buffer to contain this terminator.
 *
 * @return 0 on success or -1 if len is beyond allocated buffer
 */
extern int gh_buf_set_len(gh_buf *buf, size_t len);
