...
int gh_buf_try_grow(gh_buf *buf, size_t target_size, bool mark_oom)
{
	 ...
	 * to fit our target size; the overflow check here
	 * is overly conservative, but that's OK. If we're
	 * getting to even half of size_t, something is
	 * going horribly wrong. */
	while (new_size < target_size) {
		if (unsigned_mult_overflows(new_size, 2)) {
			if (mark_oom) {
				if (buf->ptr && buf->ptr != gh_buf__initbuf)
					free(buf->ptr);
				buf->ptr = gh_buf__oom;
			}
			return -1;
...
int gh_buf_set_len(gh_buf *buf, size_t len)
{
	if (len >= buf->asize)
		return -1;
